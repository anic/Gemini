...
let libs = {};
libs.title = function (title) {
    title = title || 'Yearning SQL审核平台';
    window.document.title = title
};

libs.random = function random(lower, upper) {
    return Math.floor(Math.random() * (upper - lower)) + lower;
};

libs.mode = function (obj) {
    let oc = {};
...
libs.openPage = function (vm, name) {
    ...
    libs.taglist(vm, name)
};

libs.taglist = function (vm, name) {
...
libs.sameMerge = function (obj, merge, el) {
...
libs.concat = function (arr1, arr2) {
    let arr = arr1.concat();
